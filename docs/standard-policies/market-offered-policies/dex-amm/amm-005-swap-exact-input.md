...
> 사용자가 Exact-Input(판매할 토큰의 수량을 정하는 방식의) Swap을 시도할 때, 수령할 토큰의 최소 수량을 정하지 않은 경우, 그 사실을 파악하고 수정할 수 있게 경고를 표시합니다.

minAmountOut(수령할 토큰의 최소 수량)이 0으로 설정되어 있으면, MEV의 일종인 샌드위치 공격, 또는 선행 매매에 의해 시세보다 매우 낮은 금액으로 거래를 체결할 수 있습니다. 따라서 안전하게 Swap을 진행할 수 있도록 수령할 토큰의 최소 수량을 정하지 않은 경우 값을 반영할 수 있도록 경고를 표시합니다.
