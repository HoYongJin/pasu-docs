...
유동성을 공급하는 것은 단순히 토큰을 풀(Pool)에 넣는 것이 아니라, 토큰을 풀에 공급한 대가로 그 풀의 일정 지분을 소유한다는 증표를 받는 행위입니다. 그 증표인 LP Token 또는 포지션 NFT가 제3자에게 전달되면, 사용자는 유동성을 공급하더라도 그 유동성을 회수할 권리를 갖지 못하게 됩니다. 따라서 유동성을 회수할 권리가 정당하게 지급되지 않거나, 제3자에게 지급되는 경우 경고를 표시합니다.
...
서명에 사용할 지갑의 주소(`principal.address`)와 유동성 공급을 통해 토큰을 받을 주소&#x20;
