...
Permit2는 한 번의 서명으로 토큰 사용 권한을 위임하는 Uniswap의 승인 방식입니다. 이때 한도를 최대치(무제한)로 서명하면, 위임받은 상대는 그 권한이 살아 있는 동안 내 토큰 잔액 전부를 언제든 옮길 수 있습니다. 꼭 필요한 만큼만 한도를 정해 서명하도록, 무제한 서명이 감지되면 경고합니다.

#### Scope (적용 범위)

Permit2 허용량 서명(Permit2 Sign Allowance). 한도가 Permit2의 최대치(uint160 max)로 설정된 경우에 적용됩니다.
...
Permit2 서명에 담긴 허용 수량 (`context.amount`)
