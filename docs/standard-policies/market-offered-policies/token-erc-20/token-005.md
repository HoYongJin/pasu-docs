---
description: New Approval After Daily Cumulative Cap (WARN)
---

# TOKEN-005: 하루 누적 활동이 한도를 넘은 뒤의 승인 요청 시 경고

### Policy Definition (정책 정의)

> 그날 지갑에서 빠져나간 금액이 이미 $10,000을 넘은 상태에서 새 승인을 요청하는 경우 경고합니다.

하루 동안 지갑에서 나간 금액이 평소보다 많이 쌓였다는 건, 사용자가 바쁘게 거래 중이라는 뜻일 수도 있지만 누군가 지갑을 털고 있다는 신호일 수도 있습니다. 특히 승인은 나중에 자금을 빼갈 권한을 새로 여는 행위라, 이미 당일 활동이 한도를 넘긴 상황에서 또 승인이 나오면 한 번 멈춰 확인하도록 경고합니다.

#### Scope (적용 범위)

ERC-20 Approve 서명. 지갑의 당일 누적 출금액이 $10,000을 넘긴 뒤의 승인에 적용됩니다.

#### Audience (대상 사용자)

여러 dApp을 오가며 승인을 자주 하는 사용자

#### Used Data (판정에 사용될 데이터)

지갑의 당일 누적 출금액 (`windowOutflowUsd` — 활동 집계 조회 결과)

#### Policy in Code

{% code title="policy.cedar" %}
```solidity
@id("daily-cumulative-approval-cap-warn")
@severity("warn")
@reason("Your daily activity already exceeds $10k — confirm this new approval before signing")
forbid(principal, action == Token::Action::"Erc20Approve", resource)
when {
  context has custom
  && context.custom has windowOutflowUsd
  && context.custom.windowOutflowUsd.greaterThan(decimal("10000.0000"))
};
```
{% endcode %}

{% code title="manifest.json" %}
```json
{
  "id": "daily-cumulative-approval-cap-warn",
  "schema_version": 2,
  "trigger": { "where": { "action.tag": { "eq": "erc20_approve" } } },
  "policy_rpc": [{
    "id": "approval-window",
    "method": "stat_window.snapshot",
    "params": {
      "owner": "$.root.from"
    },
    "outputs": [{ "kind": "context", "field": "windowOutflowUsd", "type": "Decimal", "from": "$.result.windowOutflowUsd", "required": false }],
    "optional": true
  }],
  "custom_context": { "fields": { "windowOutflowUsd": "decimal" } }
}
```
{% endcode %}

***

**TOKEN-005: 하루 누적 활동이 한도를 넘은 뒤의 승인 요청 시 경고**\
Wallet Guardians | v.1.0.0 | 26/06/12\
\
&#xNAN;_&#x53;upported Chain: Ethereum_
