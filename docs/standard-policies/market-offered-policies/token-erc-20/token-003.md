...
토큰을 승인하면 그 상대(Spender)는 허용한 한도 안에서 언제든 내 토큰을 가져갈 수 있습니다. 상대가 이미 자금 탈취로 신고된 주소라면, 승인하는 순간 공격자에게 토큰을 빼갈 수 있는 권한을 그대로 넘기는 셈입니다. 권한을 내어주는 순간 드레인 될 수 있어, 경고에 그치지 않고 서명 자체를 막습니다.

#### Scope (적용 범위)

ERC-20 Approve 서명. 승인 상대(Spender)의 평판을 조회해 악성으로 표시된 경우에 적용됩니다.
...
승인 상대(Spender) 주소의 악성 여부 (`spenderFlagged` — 주소 평판 조회 결과)
...
@severity("deny")
@reason("Spender is flagged as a known-malicious address — approval blocked")
